...
export default function ModalBookCheck() {
    ...
    const handleSubmit = async () => {
        ...
        const formData = {
            ...
            FlightHour: selectedData.flightPlandTime, //비행소요시간
...
const InfoComponent = ({ info, handleOpenClose, seatLevel }) => {
    const handleDateFormatChange = useMemo(() => {
        return (date) => {
            ...
            const formattedDepTime = `${hour}:${minute}`;
            const formattedArrTime = `${hour}:${minute}`;
            const formattedFlightHour = `${hour}시간 ${minute}분`;
            return { formattedDate, formattedDepTime, formattedArrTime, formattedFlightHour };
        };
    }, []);
    ...
    const { formattedDate, formattedArrTime } = handleDateFormatChange(info.arrPlandTime);
    const { formattedDepTime } = handleDateFormatChange(info.depPlandTime);
    const { formattedFlightHour } = handleDateFormatChange(info.depPlandTime);

    return (
        <div className="container-book container-book-top">
            <div className="book-top-panel">
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <p className="airport-top">{info.depAirportNm}</p>
                    <img src={book_arrow} height={'30px'} width={'30px'} />
                    <p className="airport-top">{info.arrAirportNm}</p>
                    <button className="button-search-book">변경</button>
                </div>
                <p className="date">{formattedDate}</p>
            </div>
            <div className="bookpanel">
                <div className="parent-container-book">
                    <span style={{ fontSize: '12px' }}>{info.airlineNm}</span>
                    <span style={{ fontSize: '12px' }}>{info.vihicleId}</span>
                    <div className="date-info">
                        <p className="time">{formattedDepTime}</p>
                        <p className="airport">{info.depAirportNm}</p>
                    </div>
                    <div className="date-info">
                        <p className="flighthour">{formattedFlightHour}</p>
                    </div>
                    <div className="date-info">
                        <p className="time">{formattedArrTime}</p>
                        <p className="airport">{info.arrAirportNm}</p>
                    </div>
                    <div className="seat-info">
                        <span style={{ fontSize: '12px' }}>잔여 {info.seatCapacity}석</span>
                    </div>
                    <div className="price-info">
                        <span>{seatLevel === "이코노미" ? info.economyCharge.toLocaleString() + "원" : info.prestigeCharge.toLocaleString() + "원"}</span>
                        <button className="handle-button-modal handle-button-cancelstyle-modal" onClick={() => handleOpenClose(info)}>선택</button>
                    </div>
                </div>
            </div>
        </div>
